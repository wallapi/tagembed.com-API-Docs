# Gemini CLI / Antigravity - build a Tagembed social widget
...
curl -sSLo GEMINI.md https://raw.githubusercontent.com/wallapi/tagembed.com-API-Docs/main/prompts/TAGEMBED_CONTEXT.md
```
...
curl.exe -sSLo GEMINI.md https://raw.githubusercontent.com/wallapi/tagembed.com-API-Docs/main/prompts/TAGEMBED_CONTEXT.md
```
...
Gemini follows instructions literally - the prompt below names the only two
questions it may ask (theme, then language) and moves the token to the end
...
```
Build the Tagembed social widget described in GEMINI.md and llms.txt in this folder. First open the theme picker from the theme catalogue in my browser - the thumbnails page itself, not a list of names - and ask which theme I want; then ask which language I want the server code in (any: Python, PHP, Node.js, Go, ...). One question per reply, and start building on my second answer.
